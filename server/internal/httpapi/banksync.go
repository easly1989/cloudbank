...
func (h *bankSyncHandlers) setSchedule(w http.ResponseWriter, r *http.Request) {
	wl, _ := walletFromContext(r.Context())
	id, ok := h.connID(w, r)
	if !ok {
		return
	...
	if err := h.svc.SetSchedule(r.Context(), wl.ID, id, body.Hour, body.Days); err != nil {
		h.writeErr(w, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (h *bankSyncHandlers) clearHistory(w http.ResponseWriter, r *http.Request) {
	wl, _ := walletFromContext(r.Context())
	id, ok := h.connID(w, r)
	if !ok {
		return
	}
	if err := h.svc.ClearHistory(r.Context(), wl.ID, id); err != nil {
		h.writeErr(w, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
